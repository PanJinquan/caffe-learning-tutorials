...
# 当param=learned_param网络可以学习
learned_param = [weight_param, bias_param]
# 当param=frozen_param网络不可学习(但最后一层是可以学习)
frozen_param = [dict(lr_mult=0)] * 2#只有最后一层是可学习的
...
if __name__=="__main__":
    ...
    生成Solver
    用上面两种网络配置文件生成三种网络。
    1.用caffenet的参数预训练作为参数初始值，然后冻结其他层，只留下最后一层全连接层可训练。
    2.caffenet的参数作为初始值，不冻结层，所有层均可训练
    3.没有初始值，只用了caffenet的结构，用自己的数据重新训练
    """
    # 1.仅最后一层可以学习,其他都不可以学习
    pretrained_frozen_solver = get_solver(train_file_frozen, test_file_frozen)
    # 2.所有层都可以学习,并且所有层的参数都会重新初始化,重新开始学习(相当于重新训练模型)
    untrained_solver = get_solver(train_file_learn, test_file_learn)
    # 3.所有层都是可以学习,并且所有层的参数是继承了预训练模型的参数
