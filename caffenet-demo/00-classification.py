...
We'll compare CPU and GPU modes and then dig into the model to inspect features
and the output.
'''
'''
### 1. Setup
...
Our default CaffeNet is configured to take images in BGR format. (caffe默认的图像格式是BGR)
Values are expected to start in the range [0, 255] and then have the mean ImageNet pixel value subtracted from them. 
In addition, the channel dimension is expected as the first (_outermost_) dimension.
As matplotlib will load images with values in the range [0, 1] in RGB format with the channel as the _innermost_ dimension,
 we are arranging for the needed transformations here.
*设置输入预处理。（我们将使用Caffe的`caffe.io.Transformer`来执行此操作，
 但此步骤独立于Caffe的其他部分，因此可以使用任何自定义预处理代码）。
我们的CaffeNet默认配置为获取以BGR格式的图像,值在[0,255]范围内，然后从中减去ImageNet平均像素值。 
此外，通道维度应该是第一个（_outermost_）维度。
 由于matplotlib将以RGB格式加载范围为[0,1]的值的图像，并且通道为_innermost_维度，因此我们在此处安排所需的转换。
'''
...
image = caffe.io.load_image('images/cat.jpg')
...
labels_file =  'models/synset_words.txt'
